function bulkFret
    ...
    function drawCells(cells)
        data=doubles; %This will need to be rewritten to make data be the data _as we would like to display it_.
        X=data(1:end,1);
        Xi=min(X):0.2:max(X);
        plot(graph,[600,600],[0,0])
        bufferSize=0.05;
        axis([min(X),max(X),min(min(data(1:end,3:end)))-bufferSize*(max(max(data(1:end,3:end)))-min(min(data(1:end,3:end)))),(1+bufferSize)*max(max(data(1:end,3:end)))]); %broken.  Not sure how.
        for i=1:size(cells,1)
            cells(i);
            if and(removeBackgrounds,backgrounds(cells(i))~=0) %Temporary, should be made redundent
                                                               %by the way the initialization of data is handled
